...
_DP_ORDER = {8.0: 4, 4.0: 5, 2.0: 6, 1.0: 7}


def _order(record):
    if record["arch"] in _ARCH_ORDER:
        return _ARCH_ORDER[record["arch"]]
    return _DP_ORDER.get(record["eps_target"], 8)


def _labels(record):
    if record["arch"] == "centralized":
        return "centralised\n(pooled, no reg.)"
    if record["arch"] == "centralized_reg":
        return "centralised\n(pooled, L2)"
    if record["arch"] == "federated":
        return "federated\n(no DP, no reg.)"
    if record["arch"] == "federated_reg":
        return "federated\n(no DP, L2)"
...
def tradeoff_figure(records, highlight_key=None):
    ...
    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(11.5, 7.0), sharex=True,
    ...
    ax2.set_xticks(xs)
    ax2.set_xticklabels(ticks, fontsize=9.5, color=COLOR_INK)
    _tidy(ax2)
    ...
    fig.suptitle(f"Privacy vs utility trade-off across {len(items)} configurations",
                 fontsize=14, y=0.975, x=0.5, color=COLOR_INK)
    fig.text(0.5, 0.925,
             "Federated learning + DP-SGD across three UAE health authorities  ·  "
             "3 seeds per configuration  ·  δ = 1e-5  ·  L2 columns show a fair, "
             "regularised non-private baseline",
...
def curve_figure(record):
    ...
    if record["arch"] in ("centralized", "centralized_reg"):
